...
Final project
This Python file contains functions to handle
the physics of collisions between objects in a 2D environment.
These objects can have different masses, shapes, and velocities.
The functions calculate the resulting velocities
and impulses from both central and non-central collisions.
...
def central_collision(m1, m2, v1, v2, k):
    ...
    Returns:
        tuple: Velocities after collision for both
        objects and the impulse magnitude
    """
...
def non_centric_collision(a, b, dir):
    ...
    v_a_p, v_b_p, i = central_collision(
        a.mass, b.mass, _v_a_p, _v_b_p, a.k * b.k)
    ...
    a.father.x_dot, a.father.y_dot = util.vector_plus(
        (a.father.x_dot, a.father.y_dot), vector_dva)
    b.father.x_dot, b.father.y_dot = util.vector_plus(
        (b.father.x_dot, b.father.y_dot), vector_dvb)
...
def easy_collision(a, b, dir):
    ...
    if a.w == 0:
        v_a = (v_a_p, v_a_v)
        a.father.x_dot, a.father.y_dot = util.transform_reference_frame(
            v_a, -theta)
    else:
...
def rotate_after_collision(a, i, vector1, vector2):
    ...
    Parameters:
        ...
        vector1 (tuple):
        Vector from the core to the collision point
        vector2 (tuple):
        Vector from the core to the collision point on the other object
    """
    min_m = 0
    kk = 1
    theta1 = util.get_theta_of_vector(vector1)
    theta2 = util.get_theta_of_vector(vector2)
    theta = math.fabs(util.get_norm_of_vector(vector1)
    ...
    moment = (i * theta + min_m) * moment_direction * kk
