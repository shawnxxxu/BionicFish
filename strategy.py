...
def strategy(a):
    ...
    if a.name == "robotfish":
        ...
        a_w_dot_direction = get_direction_of_theta_to_theta(
            a.theta, aiming_theta)

        # If the robot fish is pointing in the aiming direction and has low
        # velocity
        if math.fabs(a.theta - aiming_theta) < 0.1 and a.v < 20:
            a_v_mode = 2  # Accelerate
            a_w_mode = 0  # Do not rotate

        # If the robot fish is pointing in the aiming direction and has
        # sufficient velocity
